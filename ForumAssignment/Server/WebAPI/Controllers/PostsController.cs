...
public class PostsController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<OutputPostDTO>> CreatePost([FromBody] CreatePostDTO request)
    {
        try
        {
            ...
            return Created($"/api/posts/{created.Id}", new OutputPostDTO
            {
                Id = created.Id,
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<OutputPostDTO>> GetSinglePost([FromRoute] int id)
    {
        try
        {
            var post = await postRepository.GetSingleAsync(id);
